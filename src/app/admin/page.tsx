...
import { Users, FileSpreadsheet, Activity, AlertCircle, Settings, Layout, Database } from 'lucide-react';
...
async function getAdminStats() {
  ...
  const [users, sheets, syncs, errors] = await Promise.all([
    ...
    adminClient.from('sheets').select('id', { count: 'exact' }),
    adminClient.from('sync_logs').select('id', { count: 'exact' }),
    adminClient.from('sync_logs').select('*').eq('status', 'failed').limit(5),
  ]);
  return { userCount: users.count || 0, sheetCount: sheets.count || 0, syncCount: syncs.count || 0, recentErrors: errors.data || [] };
}
...
export default async function AdminDashboard() {
  ...
  if (profile?.role !== 'admin') return <div className="p-8 text-center">Admin only</div>;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex">
      ...
      <main className="flex-1 p-8">
        <div className="mb-8">
          ...
          <p className="text-slate-500 mt-1">System overview and controls</p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { label: 'Total Users', value: stats.userCount, icon: Users },
            { label: 'Total Sheets', value: stats.sheetCount, icon: FileSpreadsheet },
            { label: 'Total Syncs', value: stats.syncCount, icon: Activity },
            { label: 'Recent Failures', value: stats.recentErrors.length, icon: AlertCircle },
          ].map((stat, i) => (
            <div key={i} className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
              <stat.icon className="w-8 h-8 text-slate-400 mb-4" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 overflow-hidden shadow-sm">
            <div className="p-8 border-b border-slate-100 dark:border-slate-700"><h3 className="text-xl font-bold text-slate-900 dark:text-white">Recent Sync Failures</h3></div>
            <div className="p-4">
              {stats.recentErrors.length === 0 ? (
                <div className="py-12 text-center text-slate-400 font-medium italic">No recent errors detected.</div>
              ) : (
                <div className="space-y-2">
                  {stats.recentErrors.map((err: Record<string, string>) => (
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-200 dark:border-slate-700 p-8 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              <DeployButton />
              {['Disallow Registration', 'Maintenance Mode', 'Backup Database', 'Clear Global Cache', 'Update Env Vars'].map((action, i) => (
                <button key={i} className="p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 transition duration-200">{action}</button>
              ))}
            </div>
          </div>
        </div>
