...
async function getPublicSheets() {
  ...
  const { data } = await supabase
    .from('sheets').select('*, profiles(username, display_name)')
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 py-20 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {sheets.map((sheet) => (
            <Link key={sheet.id} href={`/@${sheet.profiles?.username || 'user'}/${sheet.slug}`} className="group bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-8 hover:shadow-2xl hover:border-indigo-500/50 transition-all duration-300 flex flex-col h-full shadow-sm">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-xs font-bold uppercase tracking-widest text-slate-400">{sheet.row_count} Rows</div>
              </div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors line-clamp-2 mb-2">{sheet.title}</h2>
              <p className="text-slate-500 font-medium text-sm flex-1">by @{sheet.profiles?.username || 'anonymous'}</p>
        ...
        {sheets.length === 0 && <div className="text-center py-40"><h3 className="text-xl font-bold text-slate-400">No public sheets yet</h3></div>}
      </div>
    </div>
  );
}