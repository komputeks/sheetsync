            ...
            <div className="hidden md:flex items-center gap-6">
              ...
              {user ? (
                <div className="flex items-center gap-3">
                  <span className="text-sm text-slate-600 dark:text-slate-300 font-medium">{profile?.display_name || user.name || user.email}</span>
                  {profile?.avatar_url && (
                ...
                <button
                  ...
                  className="bg-indigo-600 text-white px-5 py-2 rounded-xl font-semibold hover:bg-indigo-700 transition shadow-lg shadow-indigo-200 dark:shadow-none"
                >
                  Sign In
                </button>
              )}
              ...
              <div className="px-4 py-4 space-y-4">
                ...
                {user && (
                  <>
                    ...
                    {profile?.role === 'admin' && (
                      <Link href="/admin" onClick={() => setMobileOpen(false)} className="block text-slate-600 dark:text-slate-300 font-medium text-indigo-600">Admin</Link>
                    )}
                  </>
                )}
                {user ? (
                  ...
                  <button onClick={() => { signIn('google'); setMobileOpen(false); }} className="w-full bg-indigo-600 text-white px-4 py-3 rounded-xl font-semibold">Sign In</button>
                )}
              </div>
            </motion.div>
          )}
